...
// Consulta l'estat actual (quin mode hi ha actiu ara mateix) i el tradueix a
// engegat/aturat comparant-lo amb VICARE_MODE_ON/VICARE_MODE_OFF de l'.env.
async function getOperatingMode(circuit = process.env.VICARE_CIRCUIT || 0) {
  const { VICARE_INSTALLATION_ID, VICARE_GATEWAY_SERIAL, VICARE_DEVICE_ID } = process.env;
  const feature = `heating.circuits.${circuit}.operating.modes.active`;
  const url = `${API_BASE}/features/installations/${VICARE_INSTALLATION_ID}/gateways/${VICARE_GATEWAY_SERIAL}/devices/${VICARE_DEVICE_ID}/features/${feature}`;
  const data = await authedRequest('get', url);
  const props = data && data.data && data.data.properties;
  const mode = props && props.value ? props.value.value : null;
  const modeOn = process.env.VICARE_MODE_ON || 'dhwAndHeating';
  const modeOff = process.env.VICARE_MODE_OFF || 'standby';
  return {
    feature,
    mode,
    isOn: mode === modeOn ? true : mode === modeOff ? false : null,
    raw: data,
  };
}
...
// Llegeix la temperatura de consigna (setpoint) del programa actiu.
// És la mateixa feature que fa servir setProgramTemperature per canviar-la;
// aquí només la consultem. El valor sol venir a properties.temperature.value.
async function getSetpointTemperature(
  program = process.env.VICARE_PROGRAM || 'normal',
  circuit = process.env.VICARE_CIRCUIT || 0
) {
  const { VICARE_INSTALLATION_ID, VICARE_GATEWAY_SERIAL, VICARE_DEVICE_ID } = process.env;
  ...
  return {
    feature,
    value: props && props.temperature ? props.temperature.value : null,
    raw: data,
  };
}
